    ...
    <ul className="categories-list text-gray-700 text-sm flex justify-between whitespace-nowrap items-center md:justify-evenly gap-2 overflow-auto">
      <li>
        <button
          ...
          className={`all-categories-btn items-center border px-[0.8rem] py-[0.5rem] ${
            selectedCategory === 'All Categories' ? 'bg-btnColor text-white' : 'border-gray-500'
      ...
      {categories.map((category) => (
        <li key={category}>
          <button
            ...
            className={`categories-btn items-center px-[0.8rem] py-[0.5rem] rounded-full flex gap-1 ${
